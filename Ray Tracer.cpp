...
void draw_debug_ray(sf::RenderWindow& window, const Point3& start, const Point3& end, sf::Color color, float window_width, float window_height) {
    // Explicitly calculate layout mapping using float casts and float literals (f)
    float screen_x = static_cast<float>(((start.x() + 2.0) / 4.0)) * window_width;
    float screen_y = static_cast<float>((1.0 - (start.y() + 1.0) / 2.0)) * window_height;
    float target_x = static_cast<float>(((end.x() + 2.0) / 4.0)) * window_width;
    float target_y = static_cast<float>((1.0 - (end.y() + 1.0) / 2.0)) * window_height;
...
int main()
{
    ...
    // Dynamic Render Config
    unsigned int num_threads = 4;
    int max_samples_per_pixel = 32;

    
    std::cout << "     C++ REAL-TIME RAY TRACER CONFIGURATION       \n";
    std::cout << "==================================================\n\n";

    std::cout << "Enter number of execution threads to use (4, 6, 8, 12): ";
    if (!(std::cin >> num_threads) || num_threads < 1) {
        num_threads = 4; // Resilient fallback default
        std::cout << "Invalid input. Defaulting to 4 threads.\n";
    }

    std::cout << "Enter maximum progressive SPP samples limit (16, 32, 64, 500): ";
    if (!(std::cin >> max_samples_per_pixel) || max_samples_per_pixel < 1) {
        max_samples_per_pixel = 32; // Resilient fallback default
        std::cout << "Invalid input. Defaulting to 32 max SPP.\n";
    }
    std::cout << "\nLaunching graphics engine sandbox environment...\n";
    std::cout << "==================================================\n\n";

    // Clear input flags before entering application context
    std::cin.clear();
    ...
    while (window.isOpen()) {
        ...
        //unsigned int num_threads = 6; // Set to exactly 6 threads for testing //std::thread::hardware_concurrency(); // threads
        ...
        // Real time Total Ray Cast Calculations tracking
        ...
        if (debug_mode) {
            ...
            draw_debug_ray(window, Point3(-10, 0, -4), Point3(10, 0, -4), sf::Color::Green, window_width, window_height);

            for (int i = -5; i <= 5; i += 2) {
                draw_debug_ray(window, lookfrom, Point3(i * 0.5, 0.0, -4.0), sf::Color::Red, window_width, window_height);
            }
        }
